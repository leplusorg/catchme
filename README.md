...
**[Install from the VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=leplusorg.catchme)**
...
1. **Where can this exception be caught?** Right-click a `throw` and see every
   handler it can reach — including ones in *callers*, after it escapes the
   current function.
...
| [`packages/core/`](packages/core) | The VS Code extension itself |
...
Then press <kbd>F5</kbd> — the *Run Extension (with Java fixture)* launch config
