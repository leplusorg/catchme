 ...
 *
 * <p>Every key here mirrors a field in {@code @leplusorg/catchme-api}; changing
 * one without the other silently breaks the bridge, so keep them in step.
 * Positions are LSP-style: <b>0-based</b> lines and characters.
 */
final class Json {
    ...
    static Map<String, Object> throwSite(
            ICompilationUnit unit, Map<String, Object> range,
            Map<String, Object> exceptionType, boolean simulated) {
        Map<String, Object> out = new LinkedHashMap<>();
        out.put("uri", uri(unit));
        out.put("range", range);
