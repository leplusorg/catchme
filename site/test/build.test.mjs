...
if (!existsSync(SITE)) {
  console.error(
    `no build output at ${SITE} — run \`pnpm --filter @leplusorg/catchme-site build\` first`,
  );
  process.exit(1);
}
...
check("resolves the stylesheet and icon under the base path", () => {
  const index = readFileSync(path.join(SITE, "index.html"), "utf8");
  assert.ok(
    index.includes('href="/catchme/styles.css"'),
    "stylesheet not prefixed",
  );
...
check("copies the stylesheet and icon", () => {
  ...
  assert.ok(
    existsSync(path.join(SITE, "assets/icon.png")),
...
check("docs page documents every command and setting in the manifest", () => {
  ...
  const manifest = JSON.parse(
    readFileSync(
      path.resolve(SITE, "../../packages/core/package.json"),
      "utf8",
    ),
  );
  const missing = [
...
check("sets a canonical URL and a description", () => {
  ...
  assert.ok(
    /<meta name="description" content=".{40,}"/.test(index),
...
check("each page declares its own canonical URL", () => {
  ...
  for (const f of html) {
    ...
    assert.ok(
      !dupe,
      `${path.relative(SITE, f)} shares a canonical with ${dupe}`,
    );
